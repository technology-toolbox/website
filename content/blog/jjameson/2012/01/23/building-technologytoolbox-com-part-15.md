...
Then I generated a key and IV and specified the Base64-encoded values in my Web.config file:
...
In the **Comments **control, I modified the logic for displaying Gravatar images next to post comments:
...
As noted in my comments in the code above, this allows greater flexibility in Subtext blog skins. For example, this is what is currently specified in the custom skin for the Technology Toolbox site (in Comments.ascx):
...
After spending a few minutes investigating the bug, I found the culprit to be ICSharpCode.SharpZipLib.dll. A little more digging and I discovered the latest version of this assembly has the **AllowPartiallyTrusterCallers
**assembly attribute.
...
It happened once on November 8th and then twice on January 11th. From my notes, I know that the November 8th instance was my own fault. I was doing a bulk update on a number of blog posts (using a little utility that I wrote) and the tool ended up creating a lock on the **subtext\_Content
**table. Ugh.
...
**Recommendation: **Consider configuring a filter in ELMAH to weed out the hacking "noise."
