...
> prompt, the transcript file remains open and you need to type {{< kbd "Stop-Transcript" >}}
> to close it.
...
Notice the log file doesn't show the `Write-Host`  	output (e.g. "{{< sample-output "Changing to TEMP folder..." >}}") -- which would make  	it much more difficult to troubleshoot an error (or bug) in a lengthy script.
...
Here is a sample log file (as viewed in Notepad), which shows the `Write-Host` messages as well as other output  	(e.g. "{{< sample-output "processed file: C:\inetpub\wwwroot\..." >}}" from icacls.exe):
