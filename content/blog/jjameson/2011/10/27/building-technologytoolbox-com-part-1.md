...
The second issue is related to the fundamental architecture of Orchard. When  I used SQL Server Profiler to inspect the database calls for one of my sample blog  pages (as described in [one of my previous posts](/blog/jjameson/archive/2010/09/03/analyzing-database-roundtrips-with-sql-server-profiler.aspx)), I observed over 40 separate round-trips to the database  just to render a *single* page. [Note that I am going off memory here from  a couple of months ago, so I could be wrong about the number -- but I seem to recall  something in the neighborhood of 42-44 round-trips to SQL Server.]
