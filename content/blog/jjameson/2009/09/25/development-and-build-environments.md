...
date: 2009-09-25T01:57:00+08:00
...
In a previous [post](/blog/jjameson/archive/2009/06/09/environment-naming-conventions.aspx), I briefly touched on the "DEV-TEST-PROD" triad of environments that         I typically recommend (at a minimum) for every organization doing any form of software         development.
...
>             necessary depending this size of a project. PROD might also refer to multiple physical
>             locations (i.e. for disaster recovery) and depending on release schedules, an additional
...
>             to easily debug and reconfigure the environment.
...
>             consideration must be made when debugging in this environment (since this will likely
>             interrupt anyone else accessing the DEV environment). Generally speaking, debugging
>             should only take place on the DEV environment when the issue cannot be reproduced
>             in a LOCAL environment. Notification should be provided to the entire team prior
>             to initiating a debugging session directly on the DEV environment (this will also
>             greatly reduce the time required to investigate the issue by avoiding concurrent
>             activities).
...
>             and assist the Development team with debugging issues in the TEST environment.
