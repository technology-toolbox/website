...
| Restore your data. | When restoring data from the Production environment to the Test environment <br>		to perform a test of the upgrade process, I used SQL Server Management Studio<br>		to add permissions for the corresponding test accounts.<br><br>For example, I added **TECHTOOLBOX\svc-tfs-test** to the <br>		corresponding TFS databases and gave it the same permissions as<br>**TECHTOOLBOX\svc-tfs**. Similarly, I added **TECHTOOLBOX\CYCLOPS-TEST$**<br>		and give it the same permissions as **TECHTOOLBOX\CYCLOPS$** <br>		(to grant acceess to services configured to run as **NT AUTHORITY\NETWORK <br>		SERVICE**).<br><br>I used the SharePoint **Mount-SPContentDatabase** cmdlet <br>		to attach the content database containing the TFS project sites and discovered<br>		a few issues with the upgraded TFS sites (more detail provided in my next <br>		blog post). |
