...
date: 2010-11-29T09:18:00-07:00
...
The process I explained before generally works with TFS 2010 when using the UpgradeTemplate.xaml         workflow (since that workflow is essentially just a wrapper around the legacy TFSBuild.proj         file). In fact, I've been using that process (with a few tweaks) for several TFS         projects since upgrading the ["Jameson Datacenter"](/blog/jjameson/2009/09/13/the-jameson-datacenter) to TFS 2010 last May.
...
Also, as noted in Jim's post, John Robbins wrote a [post](http://www.wintellect.com/CS/blogs/jrobbins/archive/2009/11/09/tfs-2010-build-number-and-assembly-file-versions-completely-in-sync-with-only-msbuild-4-0.aspx) about synchronizing the TFS build number with the assembly version.         John's approach uses a versioning scheme that is similar -- if not identical --         to the one used by the Visual Studio team, in which the [Major Version](/blog/jjameson/2009/04/02/best-practices-for-net-assembly-versioning) and [Minor Version](/blog/jjameson/2009/04/02/best-practices-for-net-assembly-versioning) are fixed (e.g. 10.0), the [Build Number](/blog/jjameson/2009/04/02/best-practices-for-net-assembly-versioning) is computed automatically from the current date, and         the [Revision](/blog/jjameson/2009/04/02/best-practices-for-net-assembly-versioning) is incremented for each build on a particular day.
...
1. The assembly version (e.g. 1.0.0.0) is specified in the [SharedAssemblyInfo.cs file](/blog/jjameson/2009/04/02/shared-assembly-info-in-visual-studio-projects) located in the same folder as the Visual Studio solution.
   Individual Visual Studio projects reference this shared file using the concept of
   ["linked files" in Visual Studio](/blog/jjameson/2009/04/01/linked-files-in-visual-studio-solutions). Note that the assembly version is not incremented
   with each build.
